...
export function renderIntegrations(ctx, shopId, data = {}, notice, err, probe = null) {
  ...
  const discrepancyCards = (data.discrepancies ?? []).map((row) => `<div class="card" style="margin:10px 0;border-color:${row.severity === 'critical' ? '#fca5a5' : '#fcd34d'}">
    ...
    ${canRetry ? `<form method="POST" action="${base}/discrepancies/${esc(row.id)}/retry">${row.entity_type === 'order' ? '<input type="hidden" name="confirm_provider_absent" value="1">' : ''}<button class="btn sm" type="submit"${row.entity_type === 'order' ? ' data-confirm="Chỉ xác nhận sau khi đã kiểm tra KiotViet không có đơn mang mã Nền Tảng của đơn này. Nếu chưa chắc, hãy đóng hộp thoại và không thử lại."' : ''}>${row.entity_type === 'order' ? 'Xác nhận chưa có đơn rồi gửi lại' : 'Thử đối soát lại'}</button></form>` : ''}
  </div>`).join('');
